...
local diffview = read("lua/plugins/diffview.lua")

check(fzf, '"ibhagwan/fzf-lua"', "configured search provider")
check(diffview, 'require("fzf-lua").fzf_exec', "configured fzf-lua branch picker")
check(diffview, '"<leader>Dr"', "configured diffview branch comparison mapping")
check(diffview, '"<leader>DB"', "configured diffview two-branch comparison mapping")
check(diffview, '"...HEAD"', "configured diffview current branch comparison")
check(diffview, '.. "..." ..', "configured diffview two-branch comparison")
check(diffview, "winbar_info = true", "configured diffview top context information")
check(diffview, '"<leader>e"', "configured diffview file panel toggle mapping")
check(diffview, "actions.toggle_files()", "configured diffview file panel toggle behavior")
check(diffview, '"wl"', "configured diffview file panel width increase mapping")
check(diffview, '"wh"', "configured diffview file panel width decrease mapping")
check(diffview, "nvim_win_set_width", "configured diffview file panel width resizing")
check(diffview, 'listing_style = "tree"', "configured diffview tree file panel")
check(diffview, "flatten_dirs = false", "configured diffview unflattened file panel paths")
check(diffview, 'folder_statuses = "only_folded"', "configured diffview folder statuses")
check(diffview, "width = 38", "configured diffview file panel width")
check(diffview, 'type = "split"', "configured diffview commit log split")
check(diffview, 'position = "bottom"', "configured diffview commit log position")
check(diffview, "height = 12", "configured diffview commit log height")
